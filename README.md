...
In the following you can either download our preprocessed data or construct them as follows:

### Pretraining

Extract ingredients (using CLIP) and titles (using SGP) from COCO, VG (filtered) and SBU:

To transform existing image-text pairs for structured image pairs:

1. Extract the "ingredients" following this repo [ViCHA](https://github.com/mshukor/ViCHA)

2. Extract the "title" using SGP from the contructed json files in step 1 using `create_titles()` from this script `preprocess/create_clip_da_captions_from_saved`


### Local and Global Contexts

Here we extract the local (ingredients) and global (titles) context used during finetuning. The approach is similar to constructing the structured datasets.

We provide the necessary scripts to extract, embedd and find the best visual concepts for each image:

* Extract the textual VCs from all the captions of the datasets: extract_keywords.py
* Embedd all textual VCs using CLIP text encoder: save_kwords_embeddings.py
* Embedd all images using CLIP visual encoder: save_image_embeddings.py
* Create the augmented dataset using: create_clip_da_captions_from_saved.py (if you don't have enough memory use create_clip_da_captions.py)

For Recipe1m+ we only consider one image for each recipe.
